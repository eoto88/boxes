...
class WallSprayCans(_WallMountedBox):
    ...
    def side(self, move=None):
        ...
        radius = diameter / 2
        shelf_depth = diameter * 2 + PADDING
        h = (shelf_depth * 2) + 20
        shelf_depth_without_radius = shelf_depth - radius
        depth = ((shelf_depth_without_radius / 2) * math.sqrt(2)) - radius + (PADDING * 2)
        ...
        top_shelf_x = -(h - 5)
        middle_shelf_x = -(h / 2) - (shelf_depth_without_radius / 2) + 10
        bottom_shelf_x = -(shelf_depth_without_radius - 20)
        self.fingerHolesAt(top_shelf_x, PADDING, shelf_depth_without_radius, 45)
        self.fingerHolesAt(middle_shelf_x, PADDING, shelf_depth_without_radius, 45)
        self.fingerHolesAt(bottom_shelf_x, PADDING, diameter, 45)

        if reinforced:
            c = (REINFORCED_MARGIN / 2) * math.sqrt(2)
            self.fingerHolesAt(top_shelf_x + c + t, PADDING + c, 15, -45)
            self.fingerHolesAt(middle_shelf_x + c + t, PADDING + c, 15, -45)
            self.fingerHolesAt(bottom_shelf_x + c + t, PADDING + c, 15, -45)

        self.move(tw, h, move)

    def shelf_with_holes(self, move=None):
        diameter = self.can_diameter
        depth = diameter * 2 + PADDING
        num_holes = self.num_holes
        width = (diameter * num_holes) + (PADDING * (num_holes + 1))
        radius = diameter / 2
        holes_x = depth - radius - PADDING
        reinforced = self.reinforced
    ...
    def shelf(self, move=None):
        ...
        tw = diameter + radius + 5

        if self.move(tw, width, move, True):
            return

        self.edges["f"](diameter)
        self.corner(90, radius)
        self.edge(width - (radius * 2))
        self.corner(90, radius)
        self.edges["f"](diameter)
